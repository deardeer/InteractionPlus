...
function FilterSettingInfo(){
	...
	Info.generateFilterSetting = function(){
	    ...
	    //3.5 
	    filterSetting.selectedGroupId = g_ObjectGroupManager.getSelectedGroupId();
	...
	Info.extractDefinedRect = function(){
		var defineRegionInfo = {
			left: Number($('#define_region_rect').attr('x')),
			top: Number($('#define_region_rect').attr('y')),
			height: Number($('#define_region_rect').attr('height')),
			width: Number($('#define_region_rect').attr('width')),
		};
	...
	Info.extractDefaultGroupInfo = function(){
		...
		for (var i = 0; i < liGroupId.length; i++) {
			...
			var groupName = g_ObjectGroupManager.getGroupNamebyId(iGroupId);
	...
	Info.extractNewCreateGroupInfo = function(){
		...
		for (var i = 0; i < liGroupId.length; i++) {
			var iGroupId = liGroupId[i];
			var groupType = g_ObjectGroupManager.getGroupType(iGroupId);
			if(groupType == 'origin' || groupType == 'default_compound'){
				continue;
			}
			var attr = g_ObjectGroupManager.getGroupAttrbyId(iGroupId);
	...
	Info.extractInvisibleGroupIdList = function(){
		return g_ObjectGroupManager.getInVisibleGroupIdList();
	}
...
//show the flag panel which is the container of the flag buttons
function addFlagPanel(show){
	if(!show){
		$("#flagpaneldiv").remove();
		return;
	}
	...
	//add the drag button
	// var DrawLTPos = $('#flagpaneldiv').offset();
	// drawDragButton('flagpaneldiv', DrawLTPos);
